...
| `product-tag` | Labels attached to products via the global Tags API; products carry `tags.publicTags.tagIds[]` and `tags.privateTags.tagIds[]` | [About the Tags API](https://dev.wix.com/docs/api-reference/business-management/tags/introduction) · [Create Tag](https://dev.wix.com/docs/api-reference/business-management/tags/create-tag) |
...
- **Product tags have a native Wix target.** Do not mark them skipped or route to CMS. Create tags via `POST /tags/v1/tags` with `fqdn: "wix.stores.catalog.v3.product"`, crosswalk the IDs, and set `tags.publicTags.tagIds[]` when creating products. Max 100 tags per FQDN. Create tags before products.
